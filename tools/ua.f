...
c	read(*,999) intpfile
	...
	call io_init_tp(intpfile,ntp,xht,yht,zht,vxht,vyht,
     &               vzht,istat,rstat)
        ...
        open(unit=7,file='ua.out')
c        open(unit=8,file='ua_hkpq.out')
 ...
 1      continue
             ...
             do i=2,nbod
                ...
                if(id.eq.ifol) then
                   ...
                   peri = a*(1.0d0-e)
                   apo = a*(1.0d0+e)
                   meanl = capom + omega + capm
                   if (meanl .gt. 360.0d0) meanl = meanl - 360.0d0
                   if (meanl .gt. 360.0d0) meanl = meanl - 360.0d0
c                   write(7,1000) t,a,e,inc,capom,omega,capm,peri,apo
                   write(7,1000) t,a,e,inc,capom,omega,capm,peri,meanl
